...
class ReviewCleaner:
    ...
    def detect_language(self) -> None:
        ...
        print(
            "--> [Language Detector] Running language detection (this may take a moment)..."
        )
    ...
    def clean_text(self) -> pd.DataFrame:
        ...
        print("--> [Cleaner] Cleaning reviews text.")
    ...
    def remove_junk(self) -> pd.DataFrame:
        ...
        # Filter
        mask = (self.df["word_count"] == 1) & (
            self.df["cleaned_review"].str.lower().str.strip().isin(WORDS)
        )
        self.df = self.df[~mask].copy()

        print(
            f"--> [Filter] Removed short reviews with just 1 meaningless word. Dropped {mask.sum()} rows."
        )
